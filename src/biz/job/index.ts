...
import { fetchJobProfile, fetchJobStatus, pauseJob } from "./services";
...
export class JobCore extends BaseDomain<TheTypesOfEvents> {
  ...
  static async New(body: { id: string }) {
    const { id } = body;
    const r = await new RequestCore(fetchJobProfile).run(id);
    if (r.error) {
  ...
  async fetchStatus() {
    const r = await new RequestCore(fetchJobStatus).run(this.id);
    if (r.error) {
      return Result.Err(r.error);
    }
    const { status } = r.data;
