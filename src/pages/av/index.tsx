...
export const AVListPage: ViewComponent = (props) => {
  ...
  const avList = new ListCore(new RequestCore(fetchAVMediaList, { process: fetchAVMediaListProcess }), {
  ...
  const avRef = new RefCore<AVMediaItem>();
  ...
  const searchBtn = new ButtonCore({
    onClick() {
      avList.search({ name: nameSearchInput.value });
    },
  });
  ...
  const mediaDeleteBtn = new ButtonInListCore<AVMediaItem>({
    onClick(record) {
      avRef.select(record);
  ...
  const transferBtn = new ButtonInListCore<AVMediaItem>({
    onClick(record) {
      if (record === null) {
        return;
      }
  ...
  const [state, setState] = createSignal(avList.response);
  ...
  view.onShow(() => {
    ...
    avList.deleteItem((av) => {
      if (av.id === deleteMovie.movie_id) {
        return true;
      }
      return false;
    });
  ...
  scrollView.onReachBottom(async () => {
    await avList.loadMore();
    scrollView.finishLoadingMore();
  });
  avList.onStateChange((nextState) => {
    setState(nextState);
  });
  ...
  return (
    <>
      <ScrollView store={scrollView} class="h-screen p-8">
        <h1 class="text-2xl">AV列表({state().total})</h1>
        <div class="mt-8">
            ...
            >
              <div class="space-y-4">
                <For each={state().dataSource}>
                  {(av) => {
                    const { id, name, overview, poster_path, air_date, vote_average, persons } = av;
                    const tips = av.tips || [];
                    return (
                      <div
                        class="rounded-md border border-slate-300 bg-white shadow-sm cursor-pointer"
                        onClick={() => {
                          history.push("root.home_layout.av_profile", { id });
                        }}
                      >
                        <div class="flex">
                          <div class="overflow-hidden mr-2 rounded-sm">
