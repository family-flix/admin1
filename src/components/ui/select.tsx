...
export const Select = (props: { store: SelectCore<any>; position?: "popper" } & JSX.HTMLAttributes<HTMLElement>) => {
  ...
  return (
    <SelectPrimitive.Root store={store}>
      ...
      >
        <SelectPrimitive.Value store={store} />
      ...
      <SelectPrimitive.Content
        classList={{
          "relative z-50 min-w-[8rem] overflow-hidden rounded-md border bg-popover text-popover-foreground shadow-md bg-w-bg-0":
            true,
            ...
            {(opt) => {
              return (
                <SelectPrimitive.Option
                  ...
                  store={opt}
                  class={cn(
                    "relative flex w-full cursor-default select-none items-center rounded-sm p-2 text-sm outline-none data-[highlighted]:bg-slate-100 focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50"
                  )}
                >
                  <span class="absolute right-2 flex h-3.5 w-3.5 items-center justify-center">
                    <SelectPrimitive.ItemIndicator store={opt}>
                      <Check class="h-4 w-4" />
                    </SelectPrimitive.ItemIndicator>
                  </span>
                  <SelectPrimitive.OptionText store={opt}>{opt.text}</SelectPrimitive.OptionText>
                </SelectPrimitive.Option>
              );
            }}
          </For>
